...
void combine(int a, int b)
{
    ...
    while(DT[curr_a].parent > 0) curr_a=DT[curr_a].parent;
    while(DT[curr_b].parent > 0) curr_b=DT[curr_b].parent;
    if(curr_a != curr_b){
        if(DT[curr_a].parent > DT[curr_b].parent) {DT[curr_b].parent += DT[curr_a].parent; DT[curr_a].parent = curr_b;}
        else {DT[curr_a].parent += DT[curr_b].parent; DT[curr_b].parent = curr_a;}
    }
}
...
void dfs(void)
{
    if(!dfs_flag)
	{
		...
		{
            if(DT[route[tail]].near[index] && !visited[DT[route[tail]].near[index]])
			{
                route[tail+1] = DT[route[tail]].near[index];++tail;
...
int main()
{
    ...
    for(int index = 1; index <= num; index++) DT[index].parent = -1;
    while(scanf("%d %d", &a, &b) == 2 && flag)
	{
        ...
        switch(b)
		{
          ...
          case 1:
		  {
				...
				{
                    combine(a, a-layer+1);
                    DT[a].near[1] = a-layer+1;
                    DT[a-layer+1].near[2] = a;
                }
                break;
          }
          case 2:
		  {
           ...
           case 3:
		   {
				...
				{
                    combine(a, a+layer+1);
                    DT[a].near[3]  =a+layer+1;
                    DT[a+layer+1].near[0] = a;
                }
                break;
            }
        }
